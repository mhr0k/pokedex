...
import { injectCards } from './Cards.ts';
import { Pokemon, PokemonShort } from '../types/Pokemon.ts';
...
export async function showDetails(
  ...
  if (pokemon) {
    ...
    if (!p) {
      history.pushState({ scroll, id: pokemon.id }, '', ROOT + pokemon.name);
    } else if (p === 'replace') {
      history.replaceState({ scroll, id: pokemon.id }, '', ROOT + pokemon.name);
    }
  }
...
export function hideDetails(p?: 'pop') {
  ...
  if (details) {
    ...
    if (!p) {
      ...
      history.pushState({ scroll: scroll }, '', ROOT);
    }
  }
...
export default function Main(): HTMLElement {
  ...
  if (pathname !== ROOT) {
    showDetails(pathname.split(ROOT)[1], 'replace', main);
  } else {
    main.appendChild(cards);
